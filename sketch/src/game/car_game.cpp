...
void game_init() {
  ...
  // Draw the start screen just once
  drawStartScreen(1.0f);
...
//  MAIN LOOP
// ═══════════════════════════════════════════════════════════════
void game_loop(float ext_dt) {
  unsigned long now = millis();
  float dt = (now - lastFrameMs) / 1000.0;
  lastFrameMs = now;
  if (dt > 0.1) dt = 0.1; // Limitar delta time
  if (ext_dt > 0) dt = ext_dt; // override if provided from outside
