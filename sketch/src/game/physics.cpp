...
void handleInput(float dt) {
  ...
  // --- HARDWARE OVERRIDE (IMU) ---
  float ax = 0, ay = 0, az = 0;
  qmi8658_read_accel(&ax, &ay, &az);

  // Assuming 'ay' tilt maps to left/right steering (tune sensitivity as needed)
  if (ay > 0.1f) {
      steer = -2.0f * dt * (ay * 2.0f); // scale steer speed by tilt
  } else if (ay < -0.1f) {
      steer = 2.0f * dt * (-ay * 2.0f);
  }
